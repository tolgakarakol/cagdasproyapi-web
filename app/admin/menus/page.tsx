...
export default function MenusPage() {
  ...
  const addMenu = async () => {
    ...
    setSaving(true);
    await fetch('/api/menus', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...newMenu, order: menus.length, isVisible: true, children: [] }) });
  ...
  const toggleVisible = async (m: any) => {
    ...
    await fetch('/api/menus', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(all) });
    load();
  };
  ...
  return (
    <div>
      <div className={styles.header}>
        <div><h1 className={styles.headerTitle}>Menü Yönetimi</h1><p className={styles.headerSub}>Navigasyon menüsünü düzenleyin</p></div>
        ...
        {menus.map((m, i) => (
          <div key={m._id} className={`${styles.item} ${!m.isVisible ? styles.hidden : ''}`}>
            ...
            <div className={styles.info}>
              ...
              <span className={styles.href}>{m.href}</span>
              {m.children?.length > 0 && <span className={styles.badge}>{m.children.length} alt menü</span>}
            </div>
            <div className={styles.actions}>
              <button className={`${styles.btn} ${m.isVisible ? styles.btnOn : styles.btnOff}`} onClick={() => toggleVisible(m)}>
                <i className={`fas fa-eye${m.isVisible ? '' : '-slash'}`} />
              </button>
              <button className={`${styles.btn} ${styles.btnDel}`} onClick={() => deleteMenu(m._id)}>
